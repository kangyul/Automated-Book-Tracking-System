#include "checkOut.h"
#include "mediaContainer.h"
#include "patronList.h"
#include "mediaFactory.h"
#include "patron.h"

bool CheckOut::execute(ifstream& infile, 
MediaContainer& mContainer, PatronList& patronList, int& patId) {

    int id;
    infile >> id;

    // if patron doesn't exist, return
    if(!patronList.findPatron(id)) {
        cout << "Patron with ID: " << id << " doesn't exist." << endl;
        return false;
    }

    patId = id;

    char code;
    infile >> code;

    Media* media = nullptr;

    MediaFactory mediaFactory;
    
    bool checker = mediaFactory.createMedia(code, media);

    if(!checker) {
        return false;
    } else { 
        media->setCmdData(infile);
    }

    // retrieve media from mediaContainer
    bool findBook = mContainer.retrieve(code, media, checkOutMedia);

    if(!findBook) {
        cout << "The request book doesn't exist" << endl;
        return false;
    }

    // reduce copies by 1
    checkOutMedia->checkOut();

    return true;
}

void CheckOut::display() const {
    cout << name << ' ';
    checkOutMedia->cmdDisplay();
}

Command* CheckOut::create() const {
    return new CheckOut;
}